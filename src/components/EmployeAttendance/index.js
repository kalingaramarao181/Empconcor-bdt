...
//GET TOTAL ATTENDANCE
  ...
  const renderEmployeDetails = () => {
    ...
    for (let i of empData){
      console.log((`${i.timeout.slice(0,2)}.${i.timeout.slice(2,4)}`))
      sum = sum + parseFloat(`${i.timeout.slice(0,2)}.${i.timeout.slice(3,5)}`) - parseFloat(`${i.timein.slice(0,2)}.${i.timein.slice(3,5)}`)
    }
      return <div  className='emp-view-data-table'>
            ...
            <p className='work-data-name'><span className='work-data-value'>{`${empName}`}</span></p>
              ...
              <p className='work-data-name'>Total Working Hours: <span className='work-data-value'>{(sum.toFixed(2)+"").replace(".", ":")}</span></p>
            </div>
        ...
        return (
            <tr  className="table">
              ...
              <td  className="db-item-name">{(parseFloat(`${each.timeout.slice(0,2)}.${each.timeout.slice(3,5)}`) - parseFloat(`${each.timein.slice(0,2)}.${each.timein.slice(3,5)}`)).toFixed(2)}</td>
            </tr>
        )
      })}
      </table>
